...
void JoystickTaskFunction(void *argument);
...
int main(void)
{
  ...
  // const char* test_msg = "STM32F429I-DISC1 Started - UART Working!\r\n";
  // HAL_USART_Transmit(&huart1, (uint8_t*)test_msg, strlen(test_msg), HAL_MAX_DELAY);
  
  /* Initialize joystick */
  Joystick_Init();
  
  /* Initialize joystick debug output */
  Joystick_InitDebug();
  /* USER CODE END 2 */
  ...
  JoystickTaskHandle = osThreadNew(JoystickTaskFunction, NULL, &JoystickTask_attributes);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin : PC3 (Joystick VRx - X-axis) */
  GPIO_InitStruct.Pin = GPIO_PIN_3;
  GPIO_InitStruct.Mode = GPIO_MODE_ANALOG;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);

  /*Configure GPIO pin : PA5 (Joystick VRy - Y-axis) */
...
void JoystickTaskFunction(void *argument)
{
  /* USER CODE BEGIN JoystickTaskFunction */
  /* Infinite loop */
  for(;;)
  {
    Joystick_Task();
    osDelay(JOYSTICK_POLL_INTERVAL_MS); // Poll joystick using configured interval
  }
  /* USER CODE END JoystickTaskFunction */
}
