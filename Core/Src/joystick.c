...
static Direction last_direction = DIR_NONE;
static uint32_t last_direction_time = 0;

/* Private function prototypes */
static Direction Joystick_AnalyzeDirection(uint16_t x_val, uint16_t y_val);
...
void Joystick_Init(void)
{
    ...
    directionQueueHandle = osMessageQueueNew(8, sizeof(Direction), &directionQueue_attributes);
...
Direction Joystick_GetDirection(void)
{
    ...
    Direction current_direction = Joystick_AnalyzeDirection(x_val, y_val);
